...
async def translate(request: Request):
    ...
    invalid_languages = translator.validate_inputs(
        request.src_lang, request.tgt_lang)
    if len(invalid_languages) > 0:
        error = Error(
            f"Invalid language{'s' if len(invalid_languages) > 1 else ''}: {invalid_languages},"
            f" check /languages for a list of valid inputs"
        )
    ...
    logging.info(f"result на перевод: {result}")

    result_json = {
        **request.__dict__,
        "output_text": result,
    }
    logging.info(f"Вернулся переведенный result: {result_json}")
